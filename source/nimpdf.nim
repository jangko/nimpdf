...
proc put(p: var Page, text: varargs[string]) : bool {.discardable.} =
    result = true
    for s in items(text):
        p.content.add(s)
    p.content.add('\x0A')
  
proc put(doc: Document, text: varargs[string]) : bool {.discardable.} =
    result = true
...
proc putStream(doc: Document, text: string) : bool {.discardable.} =
    doc.put("stream")
...
proc drawText*(doc: Document; x,y: float64; text: string) : int {.discardable.} =
    ...
    if font.subType == FT_TRUETYPE:
        ...
        #var utf16 = unchecked_utf8to16(text)
        #var utf16BE = u16BE(utf16)
        doc.put("BT ",f2s(xx)," ",f2s(yy)," Td <",font.EscapeString(utf8),"> Tj ET")
    else:
        doc.put("BT ",f2s(xx)," ",f2s(yy)," Td (",text,") Tj ET")
