...
proc makeGLYPHTable*(header: Header, data: FontData): GLYPHTable =
    new(result)
...
proc EncodeGLYPHTable*(t: GLYPHTable, GID2GID: var OrderedTable[int, int]): GLYPHTable =
    ...
    var newglyf = makeGLYPHTable(makeHeader(TAG.glyf, checksum(newdata, newdata.Length()), 0, newdata.Length()), newdata)
